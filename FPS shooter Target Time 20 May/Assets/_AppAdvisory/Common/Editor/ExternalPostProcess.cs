	...
	public class ExternalPostProcess : AssetPostprocessor 
	{
		...
		private static void CheckImportedAndDeletedAssetsNetworks(string[] assets, bool isImported) {
			foreach (string asset in assets) 
			{
				...
				if (asset.Contains (ExternalPackagesResolver.AudienceNetwork)) 
				{
					ScriptingSymbolsManager.SetAudienceNetworkSymbol (isImported);
				}
				if (asset.Contains (ExternalPackagesResolver.FacebookFolder)) 
				{
					ScriptingSymbolsManager.SetFacebookSDKInstalledSymbol (isImported);
				}
				if (asset.Contains (ExternalPackagesResolver.TwitterFolder)) 
				{
					ScriptingSymbolsManager.SetTwitterSDKInstalledSymbol (isImported);
				}
			}
		}
